...
void lock_ttas() {
    // Boucle d'attente active pour éviter les appels excessifs à xchg
    while (__sync_lock_test_and_set(&lock, 1)) {
        // Attend que la variable lock soit à 0 avant de tenter un xchg
        while (lock);
    }

    // Instruction atomique pour prendre le verrou
    int expected = 1;
    asm volatile (
        "xchgl %0, %1"
...
void *thread_func(void *arg) {
    ...
    for (int i = 0; i < sections; i++) {
        lock_ttas(); // Acquérir le verrou
        critical_section(); // Section critique
        unlock_ttas(); // Libérer le verrou
    }
    return NULL;
}
